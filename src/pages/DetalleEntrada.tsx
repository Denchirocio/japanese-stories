...
import { entryTypeBadgeClass, entryTypeLabel } from '../lib/entryDisplay'
...
export function DetalleEntrada({
  ...
  const entryType = entry.type ?? 'daily'
  ...
  useEffect(() => {
    ...
    getEntriesForDate(entry.date).then((entries) => {
      if (cancelled) return
      const other = entries.find((e) => (e.type ?? 'daily') === 'daily' && e.attempt !== entry.attempt)
      setSibling(other ?? null)
    })
    return () => {
      cancelled = true
    }
  }, [entry.date, entry.attempt, entryType])
