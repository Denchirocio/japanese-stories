...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const verbsText = prompt.verbs.map((w) => `${w.word} (${w.translation}, ${w.form})`).join('、')
    const adjectivesText = prompt.adjectives.map((w) => `${w.word} (${w.translation}, ${w.type}形容詞)`).join('、')
